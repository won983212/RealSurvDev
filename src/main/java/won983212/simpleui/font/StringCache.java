...
public class StringCache
{
    ...
    private void makeColorTable() {
		...
		for (int i = 0; i < 32; ++i) {
			...
			colorTable[i] = 255 << 24 | (k & 255) << 16 | (l & 255) << 8 | i1 & 255;
		}
	}
    ...
    public int renderString(String str, int startX, int startY, int initialColor, boolean shadowFlag)
    {
        ...
        BufferBuilder sb = tessellator.getBuffer();
        sb.begin(7, DefaultVertexFormats.POSITION_TEX);
        ...
        for(int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++)
        {
            ...
            float x1 = startX + glyphX;
            float x2 = startX + glyphX + texture.width;
            float y1 = startY + glyph.y;
            float y2 = startY + glyph.y + texture.height;

            sb.pos(x1, y1, 0).tex(texture.u1, texture.v1).endVertex();
            sb.pos(x1, y2, 0).tex(texture.u1, texture.v2).endVertex();
            sb.pos(x2, y2, 0).tex(texture.u2, texture.v2).endVertex();
            sb.pos(x2, y1, 0).tex(texture.u2, texture.v1).endVertex();
        }
        ...
        if(entry.specialRender)
        {
            ...
            sb.begin(7, DefaultVertexFormats.POSITION);
            GlStateManager.color((color >> 16 & 0xff) / 255f, (color >> 8 & 0xff) / 255f, (color & 0xff) / 255f, (color >> 24 & 0xff) / 255f);
            ...
            for(int glyphIndex = 0, colorIndex = 0; glyphIndex < entry.glyphs.length; glyphIndex++)
            {
                ...
                {
                    /* The divide by 2.0F is needed to align with the scaled GUI coordinate system; startX/startY are already scaled */
                    float x1 = startX + glyph.x - glyphSpace;
                    float x2 = startX + glyph.x + glyph.advance;
                    float y1 = startY + UNDERLINE_OFFSET;
                    float y2 = startY + UNDERLINE_OFFSET + UNDERLINE_THICKNESS;

                    sb.pos(x1, y1, 0).endVertex();
                    sb.pos(x1, y2, 0).endVertex();
                    sb.pos(x2, y2, 0).endVertex();
                    sb.pos(x2, y1, 0).endVertex();
                }
                ...
                if((renderStyle & ColorCode.STRIKETHROUGH) != 0)
                {
                    ...
                    float x1 = startX + glyph.x - glyphSpace;
                    float x2 = startX + glyph.x + glyph.advance;
                    float y1 = startY + STRIKETHROUGH_OFFSET;
                    float y2 = startY + STRIKETHROUGH_OFFSET + STRIKETHROUGH_THICKNESS;
